...
def maxSub_DC_subopt(array):
    result = maxSub_DC_subopt_helper(array)
    return result

def maxSub_DC_subopt_helper(array):
    ...
    leftMaxSum = maxSub_DC_subopt_helper(L)
    rightMaxSum = maxSub_DC_subopt_helper(R)
...
def maxSub_DC_opt(array):
    return maxSub_DC_opt_helper(array)[0]

def maxSub_DC_opt_helper(array):
    ...
    L = array[:mid]
    R = array[mid:]
    LmaxSum, LtotalSum, LmaxPrefix, LmaxSuffix = maxSub_DC_opt_helper(L)
    RmaxSum, RtotalSum, RmaxPrefix, RmaxSuffix = maxSub_DC_opt_helper(R)
...
def test1():
    ...
    print("Divide and Conquer (subopt): " + str(maxSub_DC_subopt(array)))
    print("Divide and Conquer (opt): " + str(maxSub_DC_opt(array)))
    print()
